...
const _withScopeId = (n) => (pushScopeId("data-v-f5bef8f1"), n = n(), popScopeId(), n);
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    useCssVars((_ctx) => ({
      "71e69bc3": containerHeight.value
    }));
    const props = __props;
    ...
    function findCoveredDom(domY, domX) {
      console.log(cardDrag.value.children);
      ...
      for (const item of domList) {
        ...
        if (Math.abs(relativeY) < props.height / 2 && Math.abs(relativeX) < props.width / 2 && dragDomData && item.id !== dragDomData.id) {
          console.log(item.tabIndex, item, dragDomData);
...
const CardDragSort = /* @__PURE__ */ _export_sfc(_sfc_main, [["__scopeId", "data-v-f5bef8f1"]]);
